            ...
            <tbody>
                @foreach ($cart->items as $item)
                    <tr>
                        ...
                        <td>{{ $item->mausac->Ten_MS }}</td>
                        <td>{{ $item->kichco->Ten_KC }}</td>
                        <td>
                            <form action="{{ route('cart.update', $item->id) }}"> <input type="number" name="quantity"
                                    value="{{ $item->quantity }}" style="width:80px; text-align: center"> <button
                        ...
                        <td><a href="{{ route('cart.delete', $item->id) }}" class="btn btn-sm btn-danger"
                                onclick="return
confirm('Bạn có chắc không?')">Delete</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        <div class="text-center"> <a href="{{ route('home') }}" class="btn btn-sm btn-primary">Tiếp tục mua hàng</a>
            <a href="{{ route('cart.clear') }}" class="btn btn-sm btn-danger"
                onclick="return confirm('Bạn có chắc không?')">Xóa
                hết</a> <a href="{{ route('order.checkout') }}" class="btn btn-sm btn-success">Đặt hàng</a>
        </div>
    @else
        <div class="alert alert-danger"> <button type="button" class="close" data-dismiss="alert"
                ...
                href="{{ route('shop', ['page' => 1]) }}">hãy click vào đây</a> để tiếp tục mua hàng
        </div>
    @endif
@stop()
